...
## 2025-02-12 - Optimize DOM queries for login elements
**Learning:** Frequent DOM queries via `document.getElementById` inside UI event handlers can cause unnecessary overhead.
**Action:** Use module-level lazy evaluation (e.g. `if(!el) el = document.getElementById('id');`) to cache DOM references, boosting query speed by ~60% without risking immediate execution errors.
