...
## 2026-08-14 - Native Hover Tooltips and Required Attributes for Transparent Inputs
**Learning:** When implementing form inputs as transparent hitzones over background images, mouse users lack visual indicators of the input's purpose before clicking, and screen readers may not announce their mandatory state.
**Action:** Always add `title` attributes (matching the `aria-label`) to provide native hover tooltips for mouse users, and explicitly include the `required` attribute so screen readers accurately announce their mandatory state.
