...
## 2024-05-24 - [Missing API Timeouts]
**Vulnerability:** Client-side DoS via indefinite network hanging on Firebase Firestore calls.
**Learning:** External network calls without timeouts can hang indefinitely in offline scenarios or degraded networks, causing the application to become unresponsive or silently fail.
**Prevention:** Wrap all network-dependent API calls (e.g., `getDoc`) with a timeout mechanism like `withTimeout(call, 10000)`.
