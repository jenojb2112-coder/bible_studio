...
## 2026-08-16 - Hover tooltips and required states for transparent form inputs
**Learning:** When implementing form inputs as transparent hitzones over background images, mouse users lack visual indicators of the hitzone boundaries or their specific functions, and screen readers may not announce their mandatory state.
**Action:** Always add `title` attributes (matching the `aria-label`) to provide hover tooltips for mouse users, and explicitly include the `required` attribute so screen readers accurately announce their mandatory state.
